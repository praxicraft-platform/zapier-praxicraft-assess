...
const hidden_org_case = hiddenTrigger(
  "hidden_org_case",
  "Case",
  { id: "case-uuid", title: "MCQ", label: "MCQ" },
  async (z, bundle) => labelled(await listPath(z, bundle, "/cases/"), (row) => row.title || row.id),
);

const hidden_all_case = hiddenTrigger(
  "hidden_all_case",
  "Case",
  { id: "case-uuid", title: "MCQ", label: "MCQ (org)" },
  async (z, bundle) => {
    const org = labelled(await listPath(z, bundle, "/cases/"), (row) => `${row.title || row.id} (org)`);
    let library = [];
    try {
      library = labelled(await listPath(z, bundle, "/platform-cases/"), (row) => `${row.title || row.id} (library)`);
    } catch {
      library = [];
    }
...
const hidden_assessment_case = hiddenTrigger(
  "hidden_assessment_case",
  "Case",
  { id: "case-uuid", title: "SQL joins", label: "SQL joins" },
  async (z, bundle) => {
    const slug = bundle.inputData && bundle.inputData.slug;
    if (!slug) return [];
    const rows = await listPath(z, bundle, `/assessments/${encodeURIComponent(slug)}/cases/`);
