...
function caseBody(input, mode) {
  const body = {};
  if (mode === "create") {
    body.title = input.title;
    body.case_type = input.case_type || "mcq";
    ...
    if (input.description) body.description = input.description;
    if (input.case_type) body.case_type = input.case_type;
...
function interviewBody(input, mode) {
  ...
  if (input.coding_case_ids) body.coding_case_ids = parseCommaIds(input.coding_case_ids);
  if (input.org_case_ids) body.org_case_ids = parseCommaIds(input.org_case_ids);
